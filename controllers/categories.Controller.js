...
const categories_schema = require("../models/categories.Schema");
const mongoose = require("mongoose");
const { generateDownloadLink, generateFilePathForDB, generateFileDownloadLinkPrefix } = require("../utils/utility");

module.exports = class categories_Controller extends BaseController {
  async add_categories(req, res) {
    try {
      ...
      const add_categories = new categories_schema(data);
      const Add_categories = await add_categories.save();
  ...
  async display_categories(req, res) {
    try {
      ...
      const display_categories = await categories_schema.find({ agency_id: tokenData.id, _id: _id });

      display_categories[0].img = generateFileDownloadLinkPrefix(req.localHostURL) + display_categories[0].img;
  ...
  async display_all_categories(req, res) {
    try {
      ...
      if (is_deleted === "true") {
        // console.log("123");
        result = await categories_schema.aggregate([
          {
            $match: {
        ...
        // productPaginate1 = await categories_schema.aggregatePaginate(result, option);
  ...
  async update_categories(req, res) {
    try {
      ...
      const update_categories = await categories_schema.findByIdAndUpdate({ _id: _id, agency_id: tokenData.id }, Data);
      return this.sendJSONResponse(
        res,
        "update categories",
        {
  ...
  async delete_categories(req, res) {
    try {
      ...
      const delete_categories = await categories_schema.findByIdAndUpdate({ _id: _id }, Data);
