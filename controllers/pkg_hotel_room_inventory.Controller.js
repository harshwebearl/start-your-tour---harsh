...
const room_schema = require("../models/pkg_hotel_room_inventory.Schema");
...
const fn = "pkg_hotel_room_inventory";
module.exports = class pkg_hotel_room_inventory_Controller extends BaseController {
  ...
  async display_all_room(req, res) {
    try {
      ...
      for (let i = 0; i < result.length; i++) {
        result[i].currency_img = await image_url(fn, result[i].currency_img);
      }

      const pkg_hotel_room_inventorys = {
        pkg_hotel_room_inventory: result
      };
      return this.sendJSONResponse(
        res,
        "display  pkg_hotel_room_inventory",
        {
          length: 1
        },
        pkg_hotel_room_inventorys
      );
    } catch (error) {
      if (error instanceof NotFound) {
        console.log(error); // throw error;
