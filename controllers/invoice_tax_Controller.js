...
const invoice_tax_schema = require("../models/invoice_taxSchema");
...
module.exports = class lead_status_Controller extends BaseController {
  async add_invoice_tax(req, res) {
    try {
      ...
      const add_invoice_tax = new invoice_tax_schema(data);
      const Add_invoice_tax = await add_invoice_tax.save();
  ...
  async display_invoice_tax(req, res) {
    try {
      ...
      const display_invoice_tax = await invoice_tax_schema.find({ agency_id: tokenData.id, _id: _id });
  ...
  async display_all_invoice_tax(req, res) {
    try {
      ...
      if (is_deleted === "true") {
        // console.log("123");
        result = await invoice_tax_schema.aggregate([
          {
            $match: {
        ...
        // productPaginate1 = await invoice_tax_schema.aggregatePaginate(result, option);
  ...
  async update_invoice_tax(req, res) {
    try {
      const tokenData = req.userData;
      if (tokenData === "") {
        return res.status(401).json({
      ...
      const update_lead_status = await invoice_tax_schema.findByIdAndUpdate(
  ...
  async delete_invoice_tax(req, res) {
    try {
      const _id = req.query._id;
      ...
      const delete_invoice_tax = await invoice_tax_schema.findByIdAndUpdate({ _id: _id }, data);
