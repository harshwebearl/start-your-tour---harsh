...
const location_schema = require("../models/locationsSchema");
const userSchema = require("../models/usersSchema");
const mongoose = require("mongoose");

module.exports = class location_Controller extends BaseController {
  async add_location(req, res) {
    try {
      ...
      const Add_location = new location_schema(data);
      const Add_Location = await Add_location.save();
  ...
  async display_location(req, res) {
    try {
      ...
      const display_location = await location_schema.find({ agency_id: tokenData.id, _id: _id });
  ...
  async display_all_location(req, res) {
    try {
      ...
      if (is_deleted === "true") {
        console.log("123");
        result = await location_schema.aggregate([
          {
            $match: {
        ...
        // productPaginate1 = await location_schema.aggregatePaginate(result, option);
  ...
  async update_location(req, res) {
    try {
      ...
      const update_location = await location_schema.findByIdAndUpdate({ _id: _id, agency_id: tokenData.id }, Data);
      return this.sendJSONResponse(
        res,
        "update location",
        {
  ...
  async delete_location(req, res) {
    try {
      ...
      const delete_location = await location_schema.findByIdAndUpdate({ _id: _id }, data);
