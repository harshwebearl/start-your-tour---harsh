...
module.exports = class UserController extends BaseController {
  ...
  async viewDetail(req, res) {
    try {
      ...
      const result = {
        ...
        email_address: userData.email_address,
        role: userData.role,
        photo: userData.photo, // <-- change here
        ...additionalData,
      };
  ...
  async editDetail(req, res) {
    try {
      ...
      // Multer thi upload thayel file
      let photoPath;
      if (req.file) {
        photoPath = `../public/images/users/${req.file.filename}`;
      }
      ...
      return this.sendJSONResponse(res, "User details updated successfully", { length: 1 }, { userId, photo: photoPath });
    } catch (error) {
