...
const pkg_hotel_type_schema = require("../models/pkg_hotel_typesSchema");
...
module.exports = class location_Controller extends BaseController {
  async add_hotel_type(req, res) {
    try {
      ...
      const Add_hotel_type = new pkg_hotel_type_schema(data);
  ...
  async display_hotel_type(req, res) {
    try {
      ...
      const display_hotel_type = await pkg_hotel_type_schema.find({ agency_id: tokenData.id, _id: _id });
  ...
  async display_all_hotel_type(req, res) {
    try {
      ...
      if (is_deleted === "true") {
        // console.log("123");
        result = await pkg_hotel_type_schema.aggregate([
          {
            $match: {
        ...
        // productPaginate1 = await pkg_hotel_type_schema.aggregatePaginate(result, option);
  ...
  async update_hotel_type(req, res) {
    try {
      ...
      const update_hotel_type = await pkg_hotel_type_schema.findByIdAndUpdate(
  ...
  async delete_hotel_type(req, res) {
    try {
      ...
      const delete_hotel_type = await pkg_hotel_type_schema.findByIdAndUpdate({ _id: _id }, data);
